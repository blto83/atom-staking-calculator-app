// Replacement content for: cosmos-validator-commission-explained-how-it-affects-atom-rewards
// This is a FULL RECOVERY of the original article content (~2,050 words), not a rewrite.
// Format matches the exact ContentBlock schema from src/types.ts (verbatim, confirmed).
// Bolt: replace the entire contents of
// src/content/cosmos-validator-commission-explained-how-it-affects-atom-rewards.ts
// with the array below. Export name must match whatever EducationPage.tsx imports
// (likely cosmosValidatorCommissionExplainedHowItAffectsAtomRewards — please verify
// against the actual import statement before naming the export).
//
// IMPORTANT: This content is already complete at the target word count (~2,050 words).
// Do NOT expand, shorten, paraphrase, or add new sections. Just insert as-is, fix the
// export name if needed to match the import, run build:prerender, and report the word count.

import { ContentBlock } from '../types';

export const cosmosValidatorCommissionExplainedHowItAffectsAtomRewards: ContentBlock[] = [
  {
    type: 'intro',
